...
class ModuleInstance extends InstanceBase {
	...
	build_strips() {
				...
				for (a in statActs) {
					if (!a.match(/_/)) {	// fader has extra actions
						...
						case 'fdr':
							...
							stat[path].fdr = 0.0;
							defVariables.push({
								name: strip.description + ' ' + i + ' dB',
								variableId: dvID + "_d"
							});
							defVariables.push({
								name: strip.description + ' ' + i + ' %',
								variableId: dvID + "_p"
							});
							stat[path].dvID = dvID;
							if (strip.hasRel) {
						...
						case 'name':
							...
							stat[path].dvID = dvID;
							defVariables.push( {
								name: strip.description + ' ' + i + ' ' + act.label,
								variableId: dvID
							});
							break;
						...
						for (a in busActs) {
							if (!a.match('_')) {
								...
								case 'lvl':
									...
									stat[path].lvl = 0.0;
									defVariables.push({
										name: strip.description + ' ' + i + ' to ' + busStrip.label + ' ' + bs + ' dB',
										variableId: dvID + "_d"
									});
									defVariables.push({
										name: strip.description + ' ' + i + ' to ' + busStrip.label + ' ' + bs + ' %',
										variableId: dvID + "_p"
									});
									stat[path].dvID = dvID;
									break;
		...
		for (var a in baseAct) {
			...
			case 'color':
				...
				newColor.id = a;
				newColor.type = 'advanced';
				newColor.name = 'Color of Strip';
				newColor.label = 'Set button text to Color of Strip';
				newColor.callback = function(feedback) {
			...
			case 'led':
				...
				newColor.id = a;
				newColor.type = 'advanced';
				newColor.name = 'Color on LED';
				newColor.label = 'Set button color when LED On';
				newColor.callback = function(feedback) {
			...
			case 'solo':
				...
				newMute.id = a;
				newMute.type = 'advanced';
				newMute.name = `Show Strip ${lbl}`;
				newMute.label = `Show border if Strip is Soloed`
				newMute.callback = function(feedback) {
			...
			case 'onoff':
				...
				newMute.id = a;
				newMute.type = 'advanced';
				newMute.name = `Color on Strip ${lbl}`;
				newMute.label = `Set button color if Strip ${lbl} is On`
		...
		for (a in sendAct) {
			...
			for (var sub in sendOpt) {
				...
				subAct.name = subAct.name + ' ' + sendOpt[sub].name;
				newOpts = null;
				newOn = undefined;
				...
				case 'onoff':
					...
					newOn.id = subAct.id;
					newOn.type = 'advanced';
					newOn.name = `Color on ${newAct.name} OFF`;
					newOn.label = `Set button color if ${newAct.name} is OFF`
					newOn.options.push(
						{
							type: 'colorpicker',
							label: 'Foreground color',
					...
					newOn.callback = function(feedback) {
			...
			for (cm in cmd.cmdMap) {
				...
				soloActions[actID] = {
					...
					callback: async (action) => {
						...
						arg = {
							type: 'i',
							value: setToggle(cmd, action.options.set)
						};
						self.sendOSC(cmd, arg);
						self.sendOSC(cmd, []);
					}
				...
				soloFeedbacks[actID] = {
					type: "advanced",
					name: "Solo Bus" + ch.description + " on",
					label: "Color on Solo Bus" + ch.description,
					options: [
						{
							type: 'colorpicker',
							label: 'Foreground color',
	...
	build_talk() {
		...
		newAct = {
			...
			callback: async (action) => {
				cmd = '/cfg/talk/' + action.options.bus + '/$on'
				arg = {
					type: 'i',
					value: setToggle(cmd, action.options.on)
				}
				self.sendOSC(cmd, arg);
				self.sendOSC(cmd, []);
			}
		...
		newAct = {
			...
			label: 'Enable Talkback Destination',
			options: [
				talkBus,
				talkDest,
				{
